...
// Mock: Alice's submission is still pending review — not yet on leaderboard
const MY_SUBMISSION_STATUS = "pending" as "pending" | "approved";
const MY_AUTHOR_KEY = "alice-s";

function RankBadge({ rank }: { rank: number }) {
  return (
    <span className="font-mono inline-flex items-center gap-1 rounded-full border border-feedback-success px-2.5 py-0.5">
      <span
...
export default function LeaderboardTable() {
  ...
  const showPendingBanner = isAuthenticated && !myEntry && MY_SUBMISSION_STATUS === "pending";
      ...
      {myEntry ? (
        ...
        <div className="flex items-center gap-3 border border-muted-foreground/20 bg-secondary px-4 py-2.5">
          ...
          <span className="text-sm text-muted-foreground">
            Your submission is under review and will appear here once approved.
          </span>
        </div>
      ) : null}
          ...
          <tbody>
            {shown.length === 0 ? (
              <tr>
                <td
                  colSpan={5}
                  className="px-4 py-10 text-center font-mono text-label text-muted-foreground"
                >
              ...
              shown.map((entry, i) => {
                const isMe =
                  isAuthenticated && entry.authorKey === MY_AUTHOR_KEY;
                return (
                  <tr
                    key={entry.rank}
